...
export async function GET() {
    try {
      ...
      AND marked_at::date = CURRENT_DATE AND institution_id = $1
    `, [institutionId]);
        ...
        const activeSessionsRes = await query(`
            ...
            WHERE s.is_active = TRUE AND s.institution_id = $1
        ...
        const settingsRes = await query(`SELECT value FROM attendance_settings WHERE key = 'qr_refresh_interval' AND institution_id = $1`, [institutionId]);
