...
export async function POST(request: Request) {
    try {
        ...
        const result = await query(
            `INSERT INTO announcements (title, content, category, priority, image_data, created_by, event_date) 
             VALUES ($1, $2, $3, $4, $5, $6, $7) 
             RETURNING *`,
            // @ts-ignore
            [title, content, category, priority || 'Normal', image_data, (session.user as any).id, event_date || null]
        );
