...
export function PHPSupportForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            try {
                // 1. Attempt to hit the Next.js Bridge API (which mimics PHP)
                // This works on Vercel immediately without a real PHP server
                const response = await fetch("/api/php-bridge/contact", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(payload),
                });

                if (!response.ok) throw new Error("Bridge API unreachable");

                const data = await response.json();

                if (data.status === "success") {
                    setStatus('success-php');
                    setServerInfo(data.server_software || "PHP Bridge");
                } else {
